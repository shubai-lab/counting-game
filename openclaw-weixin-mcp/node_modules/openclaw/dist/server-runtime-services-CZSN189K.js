import { n as isVitestRuntimeEnv } from "./env-DL0trrAI.js";
...
import { i as resolveChannelRestartReason, r as evaluateChannelHealth } from "./channel-health-policy-C5iGlzQd.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-CZ9fBpWc.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-D93l-nuE.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-BCsGOxEZ.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-D6QElTn4.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-Cdi4tNcX.js");
		if (stopped) return;
