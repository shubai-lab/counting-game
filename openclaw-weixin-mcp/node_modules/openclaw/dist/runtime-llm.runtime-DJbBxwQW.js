...
import { r as normalizeModelRef } from "./model-selection-normalize-B4tdZ1L4.js";
...
async function resolveAgentId(params) {
	...
	const { resolveDefaultAgentId } = await import("./agent-scope-DMAwaA81.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-DOEIgRym.js"), Promise.resolve(resolveRuntimeConfig(options))]);
