...
def parse_getvalue(getvalue_str):
    if pd.isna(getvalue_str):
        return {}
    result = {}
    parts = str(getvalue_str).split(';')
    for part in parts:
        match = re.search(r'获取途径:(\d+) 获取价值:(\d+)', part)
        if match:
            result[int(match.group(1))] = int(match.group(2))
    return result

def get_jiangxing_price(general_id):
    match = df_conf[df_conf['GeneralID'] == general_id]
    if len(match) > 0:
        parsed = parse_getvalue(match.iloc[0]['GetValue'])
        for path in [1, 2, 11]:
            if path in parsed:
                return parsed[path]
    return None
...
def find_general_price(general_name):
    exact = df_permanent[df_permanent['item_name'] == general_name]
    if len(exact) > 0:
        exact_with_price = exact[exact['price'] > 0]
        if len(exact_with_price) > 0:
            return exact_with_price['price'].min()
        return exact['price'].min()
...
# 获取正确的价格
df['正确将星价格'] = df['GeneralID'].apply(get_jiangxing_price)
df['真实定价'] = df['武将名称'].apply(find_general_price)
...
def classify_general(row):
    ...
    jiangxing_price = row['正确将星价格'] if pd.notna(row['正确将星价格']) else 0
    general_price = row['真实定价'] if pd.notna(row['真实定价']) else 0

    if '14' in codes:
        return ('限定', '高山仰止')
    if '1003' in codes or '1101' in codes:
        if general_price >= 200000: return ('限定', '珍宝>=20万')
        if general_price >= 100000: return ('史诗', '珍宝10~20万')
        if general_price >= 50000: return ('稀有', '珍宝5~10万')
        return ('限定', '珍宝<5万')
    if '1001' in codes:
        return ('史诗', '纳贤')
    if '1' in codes or '2' in codes or '11' in codes:
        if jiangxing_price >= 10000: return ('限定', '将星>=1万')
        if jiangxing_price >= 2000: return ('史诗', '将星2000~9999')
        if jiangxing_price >= 100: return ('稀有', '将星100~2000')
        return ('普通', '将星<100')
    if '1004' in codes:
        return ('传说', '祈福')
    if '5002' in codes:
    ...
    if '2002' in codes or '2001' in codes or '2004' in codes:
        if general_price >= 200000: return ('限定', '运营>=20万')
        if general_price >= 100000: return ('史诗', '运营10~20万')
        if general_price >= 1000: return ('稀有', '运营1001~10万')
        return ('普通', '运营<1000')
    if '1102' in codes:
        return ('限定', '武庙+高山仰止')
...
wb.save('武将评级合并_v29.xlsx')
print('已生成: 武将评级合并_v29.xlsx')
print()
print('品质分布:')
...
for name in ['刘备', '关羽', '张飞', '神马超', '武安国', '神陆逊']:
    ...
    if len(check) > 0:
        ...
        jx = row['正确将星价格']
        zp = row['真实定价']
        print(f'  {name}: 将星价格={jx}, 真实定价={zp}, 品质={row["品质"]}, 划分={row["划分依据"]}')