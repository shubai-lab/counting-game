...
# 过滤：排除ExType、LevelOneGeneralLampType、LevelTwoGeneralLampType都为空的武将
df['ExType_empty'] = df['ExType'].isna()
df['Lamp1_empty'] = df['LevelOneGeneralLampType'].isna()
df['Lamp2_empty'] = df['LevelTwoGeneralLampType'].isna()
all_empty = df['ExType_empty'] & df['Lamp1_empty'] & df['Lamp2_empty']
df_valid = df[~all_empty].copy()
print(f'\n过滤后有效武将数: {len(df_valid)}')
...
# 4. 读取将星价格表 (通过DropItemPack中的GeneralID直接关联)
...
def classify_general(general_id, name, lamp_type, general_ways, jiangxing_prices, dropway_names):
    """按优先级对武将进行分类"""
    ...
    # === 第三优先级：珍宝1003获取价值 ===
    # 如果有1003途径，从获取价值中获取珍宝价格
    if 1003 in source_ways:
        # 需要从operate_info中获取获取价值
        return '待手动确认', '珍宝1003需查获取价值', 'need_manual'
...
for _, row in df_valid.iterrows():
    ...
    tier, reason, source = classify_general(general_id, name, lamp_type, general_ways, jiangxing_prices, dropway_names)
    results.append({
...
# 保存结果
output_path = 'classification_final_result.json'
