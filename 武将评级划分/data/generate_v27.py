...
def classify_general(row):
    ...
    if '1' in codes or '2' in codes or '11' in codes:
        price = row['将星价格'] if pd.notna(row['将星价格']) else 0
        if price >= 10000: return ('限定', '将星>=1万')
        if price >= 2000: return ('史诗', '将星2000~9999')
        if price >= 100: return ('稀有', '将星100~2000')
        return ('普通', '将星<100')
    if '1004' in codes:
        return ('传说', '祈福')
    if '5002' in codes:
...
df_output = df_list[['武将名称', '品质', '划分依据', '真实定价', '将星价格']]
...
for col, h in enumerate(['武将名称', '品质', '划分依据', '真实定价', '将星价格'], 1):
    c = ws3.cell(1, col, h)
    c.fill, c.font, c.border, c.alignment = header_fill, header_font, thin_border, Alignment(horizontal='center', vertical='center')
...
wb.save('武将评级合并_v27.xlsx')
print('已生成: 武将评级合并_v27.xlsx')
print()
print('品质分布:')
print(df_summary.to_string(index=False))
print()
print('总计:', len(df_output), '武将')