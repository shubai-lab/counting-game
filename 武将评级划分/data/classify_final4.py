...
# 4. 读取武将道具表，建立 ItemID -> GeneralID 映射
...
for _, row in item_conf.iterrows():
    ...
    if pd.notna(gid) and pd.notna(item_id):
        item_id_to_general[int(item_id)] = int(gid)
print(f'武将道具表ItemID->GeneralID映射: {len(item_id_to_general)}')

# 5. 读取将星价格表，正确关联到GeneralID
...
for _, row in jiangxing_jiangxing.iterrows():
    drop_ids = parse_drop_item_pack(row['DropItemPack'])
    price = row['Price']
    for drop_id in drop_ids:
        if drop_id in item_id_to_general:
            gid = item_id_to_general[drop_id]
...
def classify_general(general_id, name, lamp_type, general_ways, jiangxing_prices, general_zhenbao_prices, dropway_names):
    ...
    if 1003 in source_ways and general_id in general_zhenbao_prices:
        ...
        if price >= 200000:
            ...
            return '史诗', f'zhenbao_price:{int(price)}', 'jiangxing'
        elif price >= 1001:
...
# 保存结果
output_path = 'classification_final4_result.json'
