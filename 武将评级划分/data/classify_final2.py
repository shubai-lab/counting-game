...
# 正确过滤：LevelTwoGeneralLampType不为空且GeneralID有效
...
def parse_get_value(text):
    if pd.isna(text):
        return []
    ways = []
    way_matches = re.findall(r'获取途径[：:](\d+)', str(text))
...
def parse_get_value_detailed(text):
    ...
    return way_value, list(way_value.keys())

# 构建珍宝1003的价格字典
general_zhenbao_prices = {}
for _, row in df_valid.iterrows():
    gid = row['GeneralID']
    way_value, ways = parse_get_value_detailed(row.get('GetValue', ''))
    if 1003 in ways:
...
def classify_general(general_id, name, lamp_type, general_ways, jiangxing_prices, general_zhenbao_prices, dropway_names):
    ...
    if 1003 in source_ways and general_id in general_zhenbao_prices:
        ...
        if price >= 200000:
            return '限定', f'珍宝定价{int(price)}', 'zhenbao'
        elif price >= 100000:
            return '传说', f'珍宝定价{int(price)}', 'zhenbao'
        elif price >= 10000:
            return '史诗', f'珍宝定价{int(price)}', 'zhenbao'
        elif price >= 1001:
            return '稀有', f'珍宝定价{int(price)}', 'zhenbao'
        elif price > 0:
            return '普通', f'珍宝定价{int(price)}', 'zhenbao'
...
# 保存结果
output_path = 'classification_final2_result.json'
